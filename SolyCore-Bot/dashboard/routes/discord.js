...
router.get("/login", async function (req, res) {
  if (!req.user || !req.user.id || !req.user.guilds) {
    ...
    if (!req.client.user?.id) {
      req.client.logger.debug("El cliente no está listo Redirigiendo a /login");
...
router.get("/callback", async (req, res) => {
  if (!req.query.code) {
    ...
    req.client.logger.error("No se ha podido iniciar sesión en el panel de control. Compruebe la carpeta /logs para más detalles");
