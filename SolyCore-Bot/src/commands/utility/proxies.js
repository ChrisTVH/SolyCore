...
module.exports = {
  name: "proxies",
  description: "buscar proxies. Tipos disponibles: http, socks4, socks5",
  cooldown: 5,
  category: "UTILITY",
  ...
  async messageRun(message, args) {
    ...
    if (args[0]) {
      ...
      else return message.safeReply("Tipo de proxy incorrecto. Tipos disponibles: `http`, `socks4`, `socks5`");
    }

    const msg = await message.channel.send("Buscando proxies... Espere por favor");
  ...
  async interactionRun(interaction) {
    ...
    await interaction.followUp("Buscando proxies... Espere por favor");
    const response = await getProxies(type);
...
async function getProxies(type) {
  ...
  if (response.buffer.length === 0) return "No se han podido buscar los datos. Vuelva a intentarlo más tarde";
