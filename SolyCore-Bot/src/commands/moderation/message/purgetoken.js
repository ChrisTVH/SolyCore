  ...
  async messageRun(message, args) {
    ...
    if (typeof response === "number") {
      ...
      return message.safeReply(
        "No tengo permisos para `Leer el historial de mensajes` y `Gestión de mensajes` para borrar mensajes",
        5
      );
    } else if (response === "MEMBER_PERM") {
      return message.safeReply(
        "No tienes permiso de `Lectura del historial de mensajes` y `Gestión de mensajes` para borrar mensajes",
        5
      );
