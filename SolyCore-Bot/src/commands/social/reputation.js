...
module.exports = {
    ...
    options: [
      {
        ...
        options: [
          {
            name: "user",
            description: "que el usuario compruebe la reputación de",
  ...
  async messageRun(message, args) {
    ...
    // dar
      ...
      if (!target) return message.safeReply("Por favor, indique un usuario válido al que dar reputación");
      response = await giveReputation(message.author, target.user);
    }

    //
    else {
      response = "Utilización incorrecta del comando";
    }
...
async function viewReputation(target) {
  ...
  if (!userData) return `${target.tag} aún no tiene reputación`;
...
async function giveReputation(user, target) {
  if (target.bot) return "No se puede dar reputación a los bots";
  ...
  if (userData && userData.reputation.timestamp) {
    ...
    if (diff < 24) {
      ...
      return `Puede volver a ejecutar este comando en \`${getRemainingTime(nextUsage)}\``;
    }
  }
