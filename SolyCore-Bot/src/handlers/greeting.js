...
const buildGreeting = async (member, type, config, inviterData) => {
  ...
  // seleccionar mensaje por defecto
  if (!config.content && !config.embed.description && !config.embed.footer) {
    content =
      type === "WELCOME"
        ? `Te damos la bienvenida a nuestro servidor, ${member.displayName} 🎉`
...
/**
 * Enviar un mensaje de bienvendia
 * @param {import('discord.js').GuildMember} member
 * @param {Object} inviterData
 */
...
async function sendFarewell(member, inviterData = {}) {
  ...
  // construir el mensaje de despedida
